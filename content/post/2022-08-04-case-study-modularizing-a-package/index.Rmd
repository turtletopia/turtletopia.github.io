...
categories: []
tags: ["r", "modularization", "dependencies", "dependency optimizing"]
...
You might have noticed that woodendesc consists of functions that served as a backbone of deepdep, while querying and plotting download statistics is more of an extension. There's one package we've created that was planned to be extended since the beginning: [tidysq](https://github.com/BioGenies/tidysq).

It's a package that compresses biological sequences (e.g. DNA/RNA) through coding each letter with less bits (3 in DNA/RNA case). We've included a few basic operations like reversing, subsetting, translating to amino acids, and reading a FASTA file -- the most common file format for biological sequences. We've intentionally omitted many more advanced functions, though.
...
1. **there's an optional functionality that requires additional imports or significantly increases the weight of the package** -- collect several such functionalities so that they are somewhat related, move them into a new package, and add old package to the Imports/Depends field of the new one.

Be wary of separation if the only use case for the new package is to be imported by the old one. Avoid it if there are to few functionalities for a new package. Sometimes copying a function or two isn't a sin.

Do you want to borrow [a code that shows an install prompt for missing package](https://github.com/turtletopia/woodendesc/blob/master/R/wrrr.R)?
