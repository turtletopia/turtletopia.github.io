---
title: 'Creating observers in loop. Shiny reactivity tricks: episode one'
author: Dominik Rafacz
date: '2022-07-08'
slug: creating-observers-in-loop-shiny-reactivity-tricks-episode-one
categories: ['Tutorial']
tags: ['R', 'Shiny', 'reactivity']
---
...
One more thing worth mentioning -- in R we can actually manage the time of evaluation of the code. If a function is provided an expression, there are means to 

I won't go more into the details, if you want to get new knowledge or refresh the one that you've got, you can see [a chapter in Advanced R by Hadley Wickham](https://adv-r.hadley.nz/environments.html)
...
We can also do the same with a small change. `new.env()` call can be replaced with `rlang::env()`. *rlang* is a package that overhauls the interface of operating on expressions and environments in R, as base R ways of doing it are really messy. A minor change, but on this blog we are going to demonstrate `rlang` features heavily, so you should get used to it.
